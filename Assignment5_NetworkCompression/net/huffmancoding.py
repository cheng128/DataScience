...
def huffman_encode_conv(param, name, directory):
    ...
    shape = weight.shape
    weight_reshape = weight.reshape(shape[0]*shape[1], shape[2], shape[3])

    data, data_length, all_indices, all_indptr = [], [], [], []
    form = 'csr'    
    for w in weight_reshape:
        sub_shape = w.shape
        mat = csr_matrix(w)
        data.append(mat.data)
        data_length.append(len(mat.data))
        all_indices.append(mat.indices)
        all_indptr.append(calc_index_diff(mat.indptr))
    f_data = np.concatenate(data, axis=None)
    f_all_indices = np.concatenate(all_indices, axis=None)
    f_all_indptr = np.concatenate(all_indptr, axis=None)
    f_data_length = np.concatenate(data_length, axis=None)
    f_data_length = f_data_length.astype('int32') 

    # Encode
    t0, d0 = huffman_encode(f_data, name + f'_{form}_data', directory)
    t1, d1 = huffman_encode(f_all_indices, name + f'_{form}_indices', directory)
    t2, d2 = huffman_encode(f_all_indptr, name + f'_{form}_indptr', directory)
    t3, d3 = huffman_encode(f_data_length, name + f'_{form}_data_length', directory)
...
def huffman_decode_conv(param, name, directory):
    ...
    lengths = huffman_decode(directory, name + f'_{form}_data_length', dtype='int32')

    all_mats = []
    start, end = 0, 0
    indptr_start, indptr_end = 0, 0
    for idx, w in enumerate(weight_reshape):
        matrix = csr_matrix
        end += lengths[idx]
        indptr_end += w.shape[1]
        data_piece = data[start:end]
        indices_piece = indices[start:end]
        indptr_piece = reconstruct_indptr(indptr[indptr_start:indptr_end])
        mat = matrix((data_piece, indices_piece, indptr_piece), w.shape)
        all_mats.append(mat.toarray())
        start += lengths[idx]
        indptr_start += w.shape[1]

    all_mats = np.array(all_mats)
    four_dim = all_mats.reshape(shape[0], shape[1], shape[2], shape[3])
    # Insert to model
    param.data = torch.from_numpy(four_dim).to(param.device)
