    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (data, target) in pbar:
            ...
            for name, p in model.named_parameters():
                ...
                #################################
                pruned_weight = p.data.cpu().numpy()
                gradients = p.grad.data.cpu().numpy()
                gradients = np.where(pruned_weight==0, 0, gradients)
                p.grad.data = torch.from_numpy(gradients).to(device)
